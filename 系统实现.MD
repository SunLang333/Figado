...
## 6. 移动应用打包 (Capacitor for Android)
...
1.  **安装 Capacitor CLI**:
    ...
    ```
2.  **初始化 Capacitor**:
    在 `bookapp_vuetify` 项目根目录下运行：

    ```bash
    npx cap init [appName] [appId] --web-dir=dist
    ```

    - `[appName]`: 应用的名称，例如 "Figado BookApp"。
    - `[appId]`: 应用的唯一包名，例如 "com.figado.bookapp"。
    - `--web-dir=dist`: 指定 Vue.js (Vite) 构建输出的目录。
      这将生成 `capacitor.config.json` 文件。

3.  **添加 Android 平台**:
    ```bash
    npx cap add android
    ```
    此命令会在 `bookapp_vuetify` 目录下创建一个 `android/` 文件夹，其中包含一个完整的 Android Studio 项目。

### 6.2. 构建与同步

1.  **构建 Vue.js 应用**:
    首先，需要构建 Web 应用以生成最新的静态资源到 `dist` 目录：
    ```bash
    npm run build
    ```
2.  **同步 Web 资源到 Android 项目**:
    ...
    ```
    此命令会将 `dist` 目录下的 Web 资源复制到 Android 项目的 `app/src/main/assets/public` 目录下，并更新原生项目的配置。

### 6.3. 在 Android Studio 中构建 APK
...
1.  **打开 Android 项目**:
    ...
    此命令会在 Android Studio 中打开 `bookapp_vuetify/android/` 项目。

2.  **配置签名 (Signing)**:
    要在真实设备上安装或发布到应用商店，APK 需要被签名。这通常在 Android Studio 的 "Build" > "Generate Signed Bundle / APK..."菜单中完成。需要创建一个签名密钥库 (Keystore) 并配置 `build.gradle` 文件。

3.  **构建 APK**:
    在 Android Studio 中，可以选择 "Build" > "Build Bundle(s) / APK(s)" > "Build APK(s)" 来生成 APK 文件。生成的 APK 通常位于 `android/app/build/outputs/apk/debug` (用于调试版本) 或 `android/app/build/outputs/apk/release` (用于发布版本)。

### 6.4. 原生功能集成 (示例)

Capacitor 允许通过插件访问原生设备功能。例如，项目中使用的 `mobileService` (如 `checkNetwork`, `showToast`, `takePicture`) 很可能就是通过 Capacitor 插件（如 `@capacitor/network`, `@capacitor/toast`, `@capacitor/camera`）与原生 Android API 进行交互的。

- **安装插件**: `npm install @capacitor/camera`
- **同步**: `npx cap sync`
- **在 JavaScript/TypeScript 中使用**:
  ...
  const takePicture = async () => {
    ...
    // passed to other plugins that accept file URLs.
  };
  ```

  通过这种方式，Web 应用可以无缝调用原生功能，提升用户体验。
